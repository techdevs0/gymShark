...
class PartnersList extends Component {
          ...
          customBodyRender: (val) => (
            <div className="d-flex nowrap">
              ...
              <Link
                className="ml-2"
                title="Edit"
                to={`/admin/partner/edit/${val}?lang=${this.state.selectedLang}`}
              >
  ...
  componentDidMount() {
    API.get(`/partners?lang=${this.state.selectedLang}`).then((response) => {
  ...
  handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this ?")) {
      ...
      API.delete(`/partners/${id}?lang=${this.state.selectedLang}`, {
        headers: {
